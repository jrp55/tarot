...
(defn dog-deal? [dealt-dog counter]
	(and (false? dealt-dog) (pos? counter) (zero? (mod counter 2))))
...
(defn deal-dog [deck result]
	(assoc result :dog (flatten (conj (if (zero? (count (:dog result))) [] (:dog result)) (take dog-deal-size deck)))))

(defn deal-int [deck players]
